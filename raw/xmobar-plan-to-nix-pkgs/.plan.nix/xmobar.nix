    ...
    components = {
      "library" = {
        ...
        modules = (((((((((([
          ...
          "Xmobar"
          ] ++ (pkgs.lib).optional (flags.with_xft || flags.all_extensions) "Xmobar/X11/MinXft") ++ (pkgs.lib).optionals (flags.with_inotify || flags.all_extensions) [
          "Xmobar/Plugins/Mail"
          "Xmobar/Plugins/MBox"
          ]) ++ (pkgs.lib).optional (flags.with_iwlib || flags.all_extensions) "Xmobar/Plugins/Monitors/Wireless") ++ (pkgs.lib).optional (flags.with_mpd || flags.all_extensions) "Xmobar/Plugins/Monitors/MPD") ++ (pkgs.lib).optionals (flags.with_alsa || flags.all_extensions) [
          "Xmobar/Plugins/Monitors/Volume"
          "Xmobar/Plugins/Monitors/Alsa"
          ]) ++ (pkgs.lib).optional (flags.with_datezone || flags.all_extensions) "Xmobar/Plugins/DateZone") ++ (pkgs.lib).optional (flags.with_mpris || flags.all_extensions) "Xmobar/Plugins/Monitors/Mpris") ++ (pkgs.lib).optional (flags.with_dbus || flags.all_extensions) "Xmobar/System/DBus") ++ (pkgs.lib).optional (flags.with_xpm || flags.all_extensions) "Xmobar/X11/XPMFile") ++ (pkgs.lib).optional (flags.with_weather || flags.all_extensions) "Xmobar/Plugins/Monitors/Weather") ++ (pkgs.lib).optional (flags.with_uvmeter) "Xmobar/Plugins/Monitors/UVMeter";
        hsSourceDirs = [ "src" ];
        };
      exes = {
        "xmobar" = {
          ...
          mainPath = ([
            "Main.hs"
            ] ++ (pkgs.lib).optional (flags.with_rtsopts) "") ++ (pkgs.lib).optional (flags.with_threaded) "";
          };
        };
      tests = {
        "XmobarTest" = {
